...
static int eth_pm_action(const struct device *dev, enum pm_device_action action)
{
	...
	case PM_DEVICE_ACTION_RESUME:
		...
		for (size_t n = 0; n < cfg->pclken_cnt; n++) {
			if (IN_RANGE(cfg->pclken[n].bus, STM32_PERIPH_BUS_MIN,
...
#define ETH_STM32_HAL_COMMON_DATA_DEFN(n)                                                          \
	PM_DEVICE_DT_INST_DEFINE(n, eth_pm_action);                                                \
...
#define ETH_STM32_HAL_COMMON_DT_INST_DEFN(n)                                                       \
	ETH_NET_DEVICE_DT_INST_DEFINE(n, eth_initialize, PM_DEVICE_DT_INST_GET(n), &eth##n##_data, \
		&eth##n##_config, CONFIG_ETH_INIT_PRIORITY, &eth_api, ETH_STM32_HAL_MTU)
