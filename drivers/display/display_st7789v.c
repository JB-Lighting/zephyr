				   ...
				   const enum display_orientation orientation)
{
	...
	case DISPLAY_ORIENTATION_ROTATED_90:
		buf_nocache[0] |= (ST7789V_MADCTL_MY_BOTTOM_TO_TOP | ST7789V_MADCTL_MV_REVERSE_MODE);
		x_offset = row_offset;
		y_offset = col_offset;
		break;

	case DISPLAY_ORIENTATION_ROTATED_180:
		buf_nocache[0] |= (ST7789V_MADCTL_MY_BOTTOM_TO_TOP | ST7789V_MADCTL_MX_RIGHT_TO_LEFT);
		x_offset = col_offset;
		y_offset = row_offset;
		break;

	case DISPLAY_ORIENTATION_ROTATED_270:
		buf_nocache[0] |= (ST7789V_MADCTL_MX_RIGHT_TO_LEFT | ST7789V_MADCTL_MV_REVERSE_MODE);
...
static int st7789v_lcd_init(const struct device *dev)
{
	...
	buf_nocache[0] = st7789v_get_colmod(config->pixel_format);
