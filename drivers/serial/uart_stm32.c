...
static int uart_stm32_pm_action(const struct device *dev, enum pm_device_action action)
{
	...
	case PM_DEVICE_ACTION_RESUME:
		...
		/*
		 * For startup init or when exiting low power mode, check whether UART is enabled.
		 * If not, it means the peripheral hasn't been initialized or has been powered down
		 * by the low-power mode. If suspend-to-RAM is enabled, assume the entire SoC has
		 * been powered down and do a full re-initialization. Otherwise, assume that the
		 * low-power mode shut down power to the UART but not critical peripherals (CPU,
		 * GPIO, RCC), which means we only have to reconfigure this UART instance.
		 *
...
static int uart_stm32_init(const struct device *dev)
{
	...
	/* Make sure the first call to uart_stm32_pm_action does a full initialization, because the
	 * uart could be left enabled by a bootloader.
	 */
	LL_USART_Disable(config->usart);

	/* When CONFIG_PM_DEVICE is not set, the callback is directly called with
	 * PM_DEVICE_ACTION_RESUME and the uart gets initialized there. Otherwise PM DEVICE takes
	 * care of initializing and de-init.
	 */
